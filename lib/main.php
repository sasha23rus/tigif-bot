<?
	...
	class Main
	{
		...
		{
			return Mysql::create( "localhost", "sasha23_tibot", "ssU6p9jM" )->setErrorMessagesLang( 'ru' )->setDatabaseName( "sasha23_tibot" )->setCharset( "utf8" )->setStoreQueries( false );
		}
		...
		public static function getImage( $query, $type, $start = 0 )
		{
			...
			$queryParams = [ 'cx' => '278b988d6545e4b27', 'imgType' => $imgType, 'q' => $query, 'safe' => false, 'searchType' => 'image', 'sort' => 'date:d', 'key' => 'AIzaSyDFNyTrlssWgzHFXyTVc7vArsa9lj3DC4o', 'excludeTerms' => 'мем, акция, купить, песня, домашнее, член, смешные, ебется, joyreactor, вибратор, пожелание, SEX.COM, Шмель, shemale', 'start' => ( $start > 0 ) ? $start * 10 : 0 ];
		...
		{
			$arData = array( 'method' => 'reitng', 'id' => $pic_id, 'action' => $action );
			return implode( "|", $arData );
		}
		
		public static function AdminBtns( $pic_id, $new = '' )
		{
			$keyboard[] = array( 'text' => '⬅️Prev', 'callback_data' => implode( "|", array( 'method' => 'previous', 'id' => $pic_id, 'action' => 'getprev' ) ) );
			if( $new ) {
				$keyboard[] = array( 'text' => '🆗', 'callback_data' => self::setReitingBtn( $pic_id, 'confirmnow' ) );
			}
			$keyboard[] = array( 'text' => '⛔', 'callback_data' => self::setReitingBtn( $pic_id, 'removenow' ) );
			$keyboard[] = array( 'text' => 'Next ➡', 'callback_data' => implode( "|", array( 'method' => 'next', 'id' => $pic_id, 'action' => 'getnext' ) ) );
			return $keyboard;
		}
		...
		public static function reitingBtns( $pic_id )
		{
			...
			$keyboard = array( array(
				...
				array( 'text' => ( $reitDIS > 0 ) ? '👎 ' . $reitDIS : '👎', 'callback_data' => self::setReitingBtn( $pic_id, 'dislike' ) ), array( 'text' => '#' . $pic_id, 'callback_data' => self::setReitingBtn( $pic_id, 'info' ) ), array( 'text' => ( $reitBAN > 0 ) ? ' 🚫' . $reitBAN : '🚫', 'callback_data' => self::setReitingBtn( $pic_id, 'ban' ) ),
				
				array( 'text' => ( $reitLIKE > 0 ) ? ' 👍' . $reitLIKE : '👍', 'callback_data' => self::setReitingBtn( $pic_id, 'like' ) ),
		...
		public static function setReiting( $id, $from, $action, $file )
		{
			...
			$data = array( 'ID_PIC' => $id, 'USER_ID' => $from, 'ACTION' => $action, 'TYPE' => $img['TYPE'] );
			if( $action != 'info' && $action != 'removenow' && $action != 'confirmnow' )
		...
		{
			$data = array( 'NIC' => $ar['username'], 'USER_ID' => $ar['id'], 'FIRST_NAME' => $ar['first_name'], 'LAST_NAME' => $ar['last_name'] );
		...
		{
			if( !self::checkIMGinBase( $url ) > 0 ) {
				$data = array( 'ACTIVE' => 1, 'URL' => $url, 'TYPE' => strtoupper( $type ), 'USER_CONFIRM' => 1 );
				if( !$adm ) {
					$data['ACTIVE'] = 0;
					$data['USER_CONFIRM'] = 0;
				}
		...
		public static function addContent( $type, $sendresult, $uid )
		{
			...
			$res = Main::DB();
			$dataFile = array( 'file_id' => $sendresult['file_id'], 'file_unique_id' => $sendresult['file_unique_id'], 'file_size' => $sendresult['file_size'], 'file_path' => $sendresult['file_path'], 'user_name' => $sendresult['user_name'], 'user_uid' => $sendresult['user_uid'], 'type' => $sendresult['type'], 'thumb' => $sendresult['thumb'], 'file_name' => $sendresult['file_name'], 'mime_type' => $sendresult['mime_type'], );
			$res->query( 'INSERT INTO `FILES` SET ?As', $dataFile );
			$id = $res->getLastInsertId();
			$data = array( 'ACTIVE' => 1, 'URL' => $sendresult["info"]["link"], 'INFO' => serialize( $sendresult["info"] ), 'TYPE' => strtoupper( $type ), 'FILE_ID' => $sendresult['file_id'], 'FILE' => $id, 'USER_CONFIRM' => 1 );
			if( $admin != 'Y' ) {
				$data['ACTIVE'] = 0;
				$data['USER_CONFIRM'] = 0;
			}
		...
		{
			$arData = array( 'method' => 'top', 'action' => $action );
			return implode( "|", $arData );
		}
		
		public static function TopBtns(): array
		{
			$keyboard = array( array( array( 'text' => 'TOP GIF`s all time', 'callback_data' => self::setTopBtn( 'gif_all' ) ), array( 'text' => 'TOP GIf`s week', 'callback_data' => self::setTopBtn( 'gif_7' ) ), ), array( array( 'text' => 'TOP PIC`s all time', 'callback_data' => self::setTopBtn( 'pic_all' ) ), array( 'text' => 'TOP PIC`s week', 'callback_data' => self::setTopBtn( 'pic_7' ) ) ) );
					...
					<div class="col">
						<nav class="nav nav-masthead justify-content-center">
							...
							<a class="nav-link<?= ( $_SERVER['SCRIPT_NAME'] == '/index.php' ) ? ' active' : ''; ?>"
							   href="/">Home</a>
							<a class="nav-link<?= ( $_SERVER['SCRIPT_NAME'] == '/statistic.php' ) ? ' active' : ''; ?>"
